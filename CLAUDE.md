...
- **`ImageCacheService`** + **`ArtworkCachingAudioCacheService`** — artwork is cached at exactly the moment its audio becomes local-ready. Keys come from `StableRemoteAssetKey` (path-only hash), because the server regenerates the Azure SAS query string on every call and a URL-keyed cache never hits. Bind `SongDto.AlbumArtDisplaySource`/`PersonaImageDisplaySource`, never the raw `*Url` properties.
